...
do
     printf "echo \" vast-tools align "${FASTQ}"/"${SAMPLE}".fastq -c 8 -sp Hsa --output "${OUT}" > "${OUT}"/log."${SAMPLE}" \"| qsub -l h_vmem=2G,tmem=2G,h_rt=00:30:00 -pe smp 8 -R y -S /bin/bash -wd $PWD -o ${OUT}/logp.$SAMPLE -j y "
 | sh     echo " "
     echo " "
done
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTkzNjk2MzYzNiwxOTQ0MTI5ODc1LC0xNj
